...
function AccessPortalPageInner() {
  ...
  // preserveCamera: clear the search but leave the camera where it is.
  // The bubble's own X keeps the default (false) so its behaviour is
  // unchanged; Find Comps passes true — see handleFindComparables.
  ...
  const handleFindComparables = async (tractId: string, county: string, state: string) => {
    ...
    // preserveCamera: true — clear the search WITHOUT the wide-bbox refit.
    // The plain clear queries the continental US and then fits the camera to
    // every accepted result, which zoomed the user from their tract out to
    // the whole country (owner bug 2026-08-06).
