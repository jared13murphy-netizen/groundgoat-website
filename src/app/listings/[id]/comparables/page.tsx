...
export default function ComparablesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleEmailComparables = () => {
    ...
    // Build email body
    let body = `Comparable Sales for ${listing?.county} County, ${listing?.state}\n`
    body += `Tract ${tract?.tract_number || '—'}: ${formatAcres(tract?.total_acres)} acres`
    if (tract?.tillable_acres && tract?.total_acres) {
      body += `, ${formatAcres(tract.tillable_acres)} tillable (${Math.round((tract.tillable_acres / tract.total_acres) * 100)}%)`
    }
    if (tract?.soil_rating) body += `, ${tract.soil_rating} Soil Rating`
    body += `\n\n`
    body += `--- ${selectedComps.length} Comparable${selectedComps.length !== 1 ? 's' : ''} Selected ---\n\n`

    selectedComps.forEach((comp, index) => {
      body += `${index + 1}. ${comp.county} County, ${comp.state}\n`
      body += `   Tract ${comp.tract_number || '—'} • ${formatAcres(comp.total_acres)} acres`
      if (comp.tillable_acres && comp.total_acres) {
        body += ` • ${formatAcres(comp.tillable_acres)} tillable (${Math.round((comp.tillable_acres / comp.total_acres) * 100)}%)`
      }
      body += `\n`
      body += `   Sold: ${formatDate(comp.auction_datetime || comp.auction_date)}\n`
      body += `   Price: ${comp.price_per_acre ? formatCurrency(comp.price_per_acre) + '/acre' : '—'}`
      if (comp.soil_rating) body += ` • Soil Rating: ${comp.soil_rating}`
      body += `\n\n`
    })

    body += `\n--- Generated by Ground Goat ---\nhttps://groundgoat.com`

    const subject = `Comparable Sales - ${listing?.county} County, ${listing?.state}`
    const userEmail = user?.email || ''
    const mailtoUrl = `mailto:${encodeURIComponent(userEmail)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
