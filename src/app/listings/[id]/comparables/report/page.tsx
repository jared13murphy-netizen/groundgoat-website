    ...
    <div className="min-h-screen bg-gg-black text-white pt-28">
      <div className="max-w-3xl mx-auto px-4 pb-12">
        ...
        {/* Subject Tract — must match the PDF's strip pixel-for-pixel in
            ...
            SubjectStrip (shared with the portal panels) owns the
            null-vs-0 handling and the tillable/pct fallback math. */}
        {subject && (
          <div className="bg-gg-gray-900 rounded-xl p-4 mb-6 border-l-4 border-gg-pink">
            ...
            <SubjectStrip
              variant="flex"
              totalAcres={subject.total_acres}
              tillableAcres={subject.tillable_acres}
              pctTillable={subject.pct_tillable}
              soilRating={subject.soil_rating}
              soilRatingType={subject.soil_rating_type}
              state={subject.state}
            />
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-gg-gray-900 rounded-lg p-3">
              <p className="text-xs text-gray-400 uppercase">Avg {perSoilRatingLabel(subject, comparables)}</p>
            ...
            <div className="bg-gg-gray-900 rounded-lg p-3">
              <p className="text-xs text-gray-400 uppercase">{avgSoilRatingLabel(subject, comparables)}</p>
            ...
            return (
              <div key={c.id} className="bg-gg-gray-900 rounded-xl p-4">
                ...
                <div className="grid grid-cols-4 gap-2 mt-3 text-center text-sm">
                  ...
                  <div><span className="font-bold">{fmtNum(c.soil_rating)}</span><br /><span className="text-xs text-gray-400">{soilRatingLabel(c)}</span></div>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-2 text-center text-sm">
                  ...
                  <div><span className="font-bold">{fmt(pricePerSoil)}</span><br /><span className="text-xs text-gray-400">{perSoilRatingLabel(c)}</span></div>
