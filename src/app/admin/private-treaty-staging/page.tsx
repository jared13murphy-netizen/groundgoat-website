...
function applyEditToScrapedData(original: any, form: EditForm): any {
  ...
  updated.tracts = form.tracts.map((t) => {
    ...
    // MANUAL OVERRIDE for any of acres/tillable/soil the admin actually CHANGED
    // here — Verify reads scraped/computed first, so a bare top-level edit was
    // ignored. Backend _pick() honors `manual` above all. Only changed fields
    // flagged, so untouched CLU-computed tillable/soil aren't clobbered.
...
export default function AdminPrivateTreatyStagingPage() {
  ...
  // Per-tract "has house" checkbox — PATCH into scraped_data so Verify reads it.
  ...
  const saveTractLandTypes = async (listing: StagingListing, idx: number, next: string[]) => {
    ...
    updated.tracts[idx].land_type = next[0] || null
    setListings((prev) =>
      prev.map((l) => (l.id === listing.id ? { ...l, scraped_data: updated } : l))
    )
    try {
  ...
  const handleDeleteTract = async (tractId: string, listingId: number) => {
    ...
    try {
      ...
      // Mirrors SwapStagingTractsPanel.onSwap below — clear this listing's
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex-1 p-6">
                        ...
                        <div className="flex items-start justify-between mb-4">
                          ...
                          {/* Source button — opens the listing page. Solid
                              pink for visibility against the light staging-page
                              background. (Copy URL removed 2026-06-01; the full
                              URL now shows under the company name.) */}
                          <div className="flex items-center gap-2">
                            <a
                              href={listing.source_url}
                              target="_blank"
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          ...
                          <div className="bg-white border border-gg-gray-500 rounded-lg p-3 shadow-sm">
                            <div className="flex items-center justify-between mb-1">
                              <p className="text-xs text-gg-gray-500">Asking Price</p>
                              {priceEditId !== listing.id && (
                                <button
                                  onClick={() => startPriceEdit(listing, info.askingPrice)}
                                  className="text-gg-gray-500 hover:text-gg-gray-900 transition-colors"
                                  title="Edit price"
                                >
                                  <Pencil size={12} />
                                </button>
