...
export default function PortalComparablesReportPanel({ subjectInfo, reportTracts, onRemoveTract, onClose, onView3DTerrain, onViewListing }: PortalComparablesReportPanelProps) {
  ...
  // an unrelated metric. The three price-per-X averages are acre-weighted
  ...
  const stats = useMemo(() => {
    ...
    const { avgPricePerAcre, avgPricePerTillable, avgPricePerSoil } = computeCompAverages(reportTracts)
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 left-0 bottom-0 w-[400px] z-[400] bg-black border-r border-white/10 shadow-2xl flex flex-col"
    >
      {/* Header */}
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
              ...
              <div className="text-[10px] text-gg-gray-400 mt-0.5">{avgSoilRatingLabel({ soil_rating_type: subjectInfo?.subject_soil_rating_type }, reportTracts)}</div>
            </div>
          </div>
        </div>
