              ...
              <div className="space-y-2">
                {listing.tracts.map((tract: any, index: number) => (
                  <div key={tract.id || index} className="flex justify-between items-center p-3 bg-gg-gray-800 rounded-lg">
                    <span className="text-white">Tract {tract.tract_number || index + 1}</span>
                    <span className="text-gg-gray-400">{tract.total_acres} acres • {tract.land_type}</span>
                  </div>
                ))}
              </div>
              <p className="text-gg-gray-500 text-sm mt-2">Edit tracts in the tract management section</p>
            </div>
          )}
